...
class MeshEditorClass:
    ...
    def mesh_edit(self, read_only=False):
        ...
        if read_only:
            scan_allocate = (
                lx.symbol.f_LAYERSCAN_ACTIVE | lx.symbol.f_LAYERSCAN_MARKPOLYS
            )
        if not read_only:
    ...
    def get_active_polys_by_island(self):
        ...
        visIslands = PolysByIslandClass(
            self.polygon_accessor, self.point_accessor, mark_mode_checked
        )
    ...
    def get_selected_polys_by_island(self):
        mark_mode_toCheck = self.mesh_svc.ModeCompose(
            lx.symbol.sMARK_SELECT, lx.symbol.sMARK_USER_0
        )
        mark_mode_valid = self.mesh_svc.ModeCompose(
            None,
            " ".join(
                (lx.symbol.sMARK_USER_0, lx.symbol.sMARK_HIDE, lx.symbol.sMARK_LOCK)
            ),
        )
        ...
        visitor = PolysByConnectedClass(
            self.polygon_accessor,
            self.edge_accessor,
            mark_mode_valid,
            mark_mode_checked,
        )
    ...
    def get_selected_polys_by_flood(self, i_POLYTAG=lx.symbol.i_POLYTAG_MATERIAL):
        ...
        visitor = PolysByTagFloodClass(
            self.polygon_accessor, self.edge_accessor, mark_mode_valid, i_POLYTAG
        )
        self.polygon_accessor.Enumerate(mark_mode_selected, visitor, 0)
    ...
    def vis_Evaluate(self):
        ...
        while len(outer_list) > 0:
            ...
            for p in xrange(num_points):
                self.edge.SelectEndpoints(
                    polygon_points[p], polygon_points[(p + 1) % num_points]
                )
                for e in xrange(self.edge.PolygonCount()):
                    edge_polygon_ID = self.edge.PolygonByIndex(e)
                    if (
                        edge_polygon_ID != polygon_ID
                        and edge_polygon_ID not in inner_list
                        and edge_polygon_ID not in outer_list
                    ):
    ...
    def vis_Evaluate(self):
        ...
        if this_polygon_ID not in outer_list:
            ...
            while len(outer_list) > 0:
                ...
                num_points = self.polygon.VertexCount()
                polygon_points = [
                    self.polygon.VertexByIndex(p) for p in xrange(num_points)
                ]
                    ...
                    if self.edge.test():
                        for e in xrange(self.edge.PolygonCount()):
                            ...
                            if edge_polygon_ID != polygon_ID:
                                if (
                                    edge_polygon_ID not in outer_list
                                    and edge_polygon_ID not in inner_list
                                ):
                                    self.polygon.Select(edge_polygon_ID)
                                    ...
                                    if self.polygon.TestMarks(
                                        self.mark_mode_valid
                                    ) and (
                                        set(tagString.split(";")).intersection(
                                            set(tagValues)
                                        )
                                    ):
                                        outer_list.add(edge_polygon_ID)
